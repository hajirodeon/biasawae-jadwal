...
if (empty($tapelkd))
	...
	//kelas
	$qrua = mysqli_query($koneksi, "SELECT * FROM m_smt ".
							"ORDER BY round(no) ASC");
	$rrua = mysqli_fetch_assoc($qrua);
	...
	if ($total != 0)
		{
		echo '[<a href="guru_mapel_tmp_xls.php?tapelkd='.$tapelkd.'" title="Print . . ."><img src="'.$sumber.'/img/xls.gif" width="16" height="16" border="0"></a>].
		...
		do
			...
			//pel-nya
			$quru = mysqli_query($koneksi, "SELECT * FROM m_guru_mapel ".
									"WHERE kd_tapel = '$tapelkd' ".
									"AND kd_guru = '$i_kd'");
